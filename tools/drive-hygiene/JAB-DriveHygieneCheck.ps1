...
function ConvertTo-JsonOutput {
    ...
    $totalReclaimable = ($ScanResults.UsageBreakdown | Measure-Object -Property Size -Sum).Sum

    $output = @{
        Metadata = @{
            Version = $Script:Version
            Timestamp = $timestamp
            IsAdministrator = Test-AdminPrivileges
            ComputerName = $env:COMPUTERNAME
        }
        PhysicalDisks = @($ScanResults.PhysicalDisks)
        SmartData = @($ScanResults.SmartData)
        Volumes = @($ScanResults.Volumes)
        TrimInfo = $ScanResults.TrimInfo
        UsageBreakdown = @($ScanResults.UsageBreakdown)
        Summary = @{
            TotalPhysicalDisks = $ScanResults.PhysicalDisks.Count
            TotalVolumes = $ScanResults.Volumes.Count
            TotalReclaimableBytes = $totalReclaimable
            TotalReclaimableFormatted = Format-ByteSize -Bytes $totalReclaimable
            CriticalVolumes = @($ScanResults.Volumes | Where-Object { $_.Status -eq "Critical" }).Count
            WarningVolumes = @($ScanResults.Volumes | Where-Object { $_.Status -eq "Warning" }).Count
        }
    }
